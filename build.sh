...
# connected/setu up correctly we use avrdude to flash that whole thing onto the attiny85.
DEVICE="attiny85"
PROGRAMMER="usbtiny"
...
HFUSE="0xDF"
EFUSE="0xFF"
VERIFY_FUSES="${AVRDUDE} -U lfuse:v:${LFUSE}:m -U hfuse:v:${HFUSE}:m -U efuse:v:${EFUSE}:m -qq"
WRITE_FUSES="${AVRDUDE} -U lfuse:w:${LFUSE}:m -U hfuse:w:${HFUSE}:m -U efuse:w:${EFUSE}:m -qq"
